import { z } from "zod";
import { idPart, ISOTimePart } from "./common.schema.js";
...
const scheduledAtPart = z.iso
    .datetime({
        offset: true,
        message: "올바른 날짜 형식이 아닙니다. (ISO8601 형식이 필요합니다)"
    })
    .refine((val) => new Date(val) > new Date(), "예약 시간은 현재 시간보다 미래여야 합니다.");
const isPublicPart = z.boolean("공개 여부는 필수 항목입니다. (true/false)");
const detailpart = z.enum(["true", "false"], {error: "detail은 true 또는 false여야 합니다."}).transform((val) => val === "true").optional();
