...
export const findLetterAssets = async () => {
    const [papers, fonts, stamps] = await prisma.$transaction([
        prisma.letterAssetPaper.findMany({ select: { id: true, color: true }, where: { isActive : true }}),
        prisma.letterAssetFont.findMany({ select: { id: true, font: true }, where: { isActive : true }}),
